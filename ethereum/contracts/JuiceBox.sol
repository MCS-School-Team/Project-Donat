...
contract LilJuicebox {
	...
	struct Campaign {
		...
		uint contributed; // sum we've currently received
		enum State { //state of the campaign
			CLOSED,
			OPEN,
			REFUNDING
		}
		bool claimed; //whether the managers have collected the money
	}

	/// @notice The address of the campaign
	IERC20 public immutable token; // url to the token 
    ...
    //@notice Receive balance of the contract
    function currentBalance() public view returns(uint){
        return address(this).balance;
    }

    //@notice Receive token balance of the contract
    function currentTokenBalance(address _token) public view returns(uint){
