            ...
            <div className="flex flex-col ">
                <div className="flex  justify-center  p-16">
                    <form className=" w-3/5 " onSubmit={handleSubmit}>
                        <input type="search"
                               className="w-10/12 h-12 text-[20px] border border-solid border-gray-300 rounded-l-lg  indent-2 bg-gray-100"
                               placeholder="Search by name..." value={findName} onChange={getName}/>
                        <input type="submit" className="bg-yellow-400 h-12 text-[20px] border rounded-r-lg  w-2/12"
                               value="Search"/>
                        <div className="w-full flex justify-center">
                            ...
                            <select
                                className="w-1/3 border border-solid rounded-lg indent-2 border-gray-300 bg-white mt-2 text-[20px]"
                                onChange={getCountry}>
                    ...
                    <ul className=" flex   w-[1250px]  ">
                        <div className="  flex   w-full flex-wrap ">
                            {items.map((item, index) => (
                                <li key={index}>
                                    <div
                                        className="flex bg-white items-center text-[25px] rounded-lg  p-9 ml-7 mt-3 mb-4 flex-col drop-shadow-xl">
                                        {item.image}
                                        {item.name}

                                        <button onClick={() => modal(index)}
                                                className="text-[17px] w-10/12 rounded-lg h-10 bg-yellow-300">Learn more
                                        </button>
                                    </div>
                                </li>
                            ))}
